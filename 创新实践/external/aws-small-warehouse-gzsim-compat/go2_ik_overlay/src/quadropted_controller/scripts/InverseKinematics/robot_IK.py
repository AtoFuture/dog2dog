...
class InverseKinematics:
    ...
    def inverse_kinematics(self, leg_positions, dx, dy, dz, roll, pitch, yaw):
        ...
        for i in range(4):
            ...
            # Numerical noise and aggressive gait commands can place a target
            # just outside the leg workspace.  Project those boundary cases
            # back onto the reachable workspace instead of aborting the whole
            # 60 Hz control loop with sqrt(negative_value).
            radial_squared = x**2 + y**2 - self.l2**2
            F = sqrt(max(0.0, radial_squared))
            G = F - self.l1
            H = sqrt(G**2 + z**2)
            ...
            theta4 = -atan2(sqrt(max(0.0, 1 - D**2)), D)
