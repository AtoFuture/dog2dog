...
float32                           torso_height_m   # 【新增】躯干**中点**离地高度（米），NaN=未测出
```
...
| 代表点语义 | `bbox 中心` / `底部中点` 由 G2 参数控制，**变更必须通知 G3** —— 它比坐标系更容易被悄悄改，且同样影响评测口径。<br>⚠️ **异常类（`source != "coco"`）例外**：它的 `pose` 是**躯干中点**（肩中点与髋中点的平均），**不受 `anchor` 参数控制**。躺姿下躯干中点与 bbox 底部中点差得很远，G3 若按同一套口径做关联/画图会系统性偏移 |
| `(id, stamp)` 不是唯一键 | 同一帧里同一个 `id` 可能产生**两条** `Detection3D`（一条 `source="coco"`、一条 `source="pose"`），两者 `stamp` 都是该帧的图像采集时刻。03b 原来说「靠 `(id, stamp)` 匹配抓拍图」—— **要加上 `source` 才是唯一键**。<br>（实务上影响有限：同一帧同一个人的两条消息对应的抓拍图是同一个人、同一帧的同一块 ROI。但按契约就不该假设键唯一。） |
...
| `torso_height_m` | **躯干中点**（肩中点与髋中点的 z 平均）离地高度（米），map 系下量。**G2 目前未标定其阈值、判定时未使用**，仅供 G3 分析。非异常类填 `NaN`。<br>⚠️ **口径说死**：另有一组「髋部高度」的数字在 G2 的实测文档里，两者不是同一个量 —— 将来标定阈值时别混 |
...
**`torso_height_m` 暂不参与判定**：它能区分「躺在地上」和「跪着」，但实测里
真倒地一侧量出了 1.58 m 这种不可能的值（深度测量错误）。
**没标定过的门只会误杀真倒地，所以宁可不关。**

⚠️ **启用前还差一步：口径要重标。** 这个字段发的是**躯干中点**高度，
而 G2 实测文档（`docs/倒地判据实测-方法B.md`）和 `fall_tracker.py` 里引用的
标定数字全是**髋部**高度 —— 两者差得不少。照那些数字设 `fall_max_height_m`
对的是另一个量。启用前需要：① 深度质量改善；② 按**躯干中点**重新标定。
