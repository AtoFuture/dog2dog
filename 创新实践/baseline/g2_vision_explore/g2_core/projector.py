...
MIN_VALID_DEPTH_M = 0.6
"""有效深度下限（米）。低于它的读数**不是测量值**，按无效处理。

---------------------------------------------------------------------------
为什么需要它（2026-09-18，真实 RGB-D 素材实测）

``depth_to_meters`` 对 ``16UC1`` 只把 **0** 当无效。但结构光相机的深度图里
还有**第二类无效像素**：分布在物体边缘（深度不连续处）的「飞点」，
它们**非零但极小**，于是原样穿过了那道检查，被当成 0.001~0.7 m 的合法深度。

在 ``1378_rgb_depth16.mkv``（Kinect v1，320x240）上实测，这类值占全体像素
约 **2.4%**，细粒度直方图在 **500~600 处有明显谷底**：

    1~10     0.043%      200~300   0.848%   ← 飞点峰
    10~50    0.174%      300~400   0.580%
    50~100   0.044%      400~500   0.137%
    100~200  0.387%      500~600   0.082%   ← 谷底（本常量取这里）
                         700~1000  0.113%   ← 真实测距从这里起

**它和下面那个取深度的函数是致命组合**：``sample_depth_near`` 取的是
**近端 5 分位**，而飞点恰好全是「近端的极小值」—— 只要窗口里飞点占比超过 5%，
第 5 百分位就**必定落在飞点堆里**。
实测关键帧的 35x35 窗口：

    直立的人的窗口    飞点占比 0.00% / 0.25%   → 5 分位没事，高度正确
    躺着的人的窗口    飞点占比 5.87%~9.72%     → 5 分位被污染

为什么躺姿的窗口飞点多：**躺着的人被透视压扁，肩髋关键点天然落在轮廓边缘**，
而飞点正是沿轮廓分布的；站立时躯干是大片均匀区域，窗口扣在身体内部。

后果是**离地高度整体抬高约 1 m**（深度偏小 → ``|y|`` 偏小 → 看着更靠近相机
的高度），而「床 0.6 / 地面 0.2」的真实差距只有 0.4 m —— 双峰因此不可能出现。
实测：躺姿离地高度中位 **+1.78 m**，比直立的 +1.32 m 还高，物理上不可能。

---------------------------------------------------------------------------
⚠️ 这个下限是**按相机**定的，不是普适常数

0.6 m 对应 Kinect v1（近距模式最小量程 0.5 m，默认模式 0.8 m）与这份素材的
实测谷底。**近距离作业的相机必须调小**（RealSense D435 最小量程约 0.28 m），
否则会静默丢掉近处目标 —— 与 ``fit_floor_plane`` 的 ``min_camera_height_m``
是同一类「按安装方式要改」的默认值。

交叉验证：``corner1_low`` 那份 RealSense 素材（640x480）里
**700 以下一个像素都没有**，真实测距起于 0.7 m —— 与本谷底吻合。
"""
...
def sample_depth_near(
    ...
    """在像素邻域内取**近端**深度（米），取不到返回 ``nan``。

    ---------------------------------------------------------------------------
    为什么要「近端」而不是「中位」（2026-09-18，实测驱动）

    给关键点取深度时，**单点取值和中位滤波都不够** —— 因为关键点本身可能
    就不在身体上。真实数据里的例子（frame 185916，坐姿）：

        左肩深度 1.64 m，右肩深度 3.03 m

    一个成年人的两个肩不可能差 1.39 m。成因是 pose 模型对**被遮挡的远侧关节**
    （人背对相机时的另一侧肩、髋）只能靠猜，猜出来的点落到了身体轮廓**之外**，
    逐像素取深度就取到了身后的背景。

    中位数救不了：窗口里背景占多数时中位数就是背景。

    但**人一定比背景近** —— 这是遮挡关系决定的，与姿态、场景无关。
    所以取窗口内深度的低分位数，就能稳定地拿到人的那一层。

    实测（57 帧真实数据，肩宽的三维中位 / 四分位距，真值 0.40 / 小）::
        ...
        11x11 中位        0.45 / 0.90
        21x21 中位        0.39 / 0.94
        **本函数 35x35 p5** **0.24** / **0.13**   ← 离散度降了 7 倍

    那为什么不干脆取更大的分位数（偏置更小）？**实测过，不能**
    （2026-09-18，同一份 57 帧数据，滤掉飞点之后重测）::
    ...
    调大分位数只把偏置从 -37% 改善到 -22%（p25），代价是**离散度恶化 4.2 倍**、
    质检留存掉三分之一、同侧深度差的 p90 从 0.53 涨到 1.22。
    最后那一列是关键：0.53 本来就压在质检门（0.40）附近，
    涨到 1.22 意味着**「关键点落到身体外时抓人而不是抓背景」这个能力崩了**。

    **「偏置但精确」还救得回来** —— 质检门用的是**区间**，把区间挪一挪就行；
    **「噪声大」救不回来**。所以 p5 这个选择是对的，**不要调大**。

    ---------------------------------------------------------------------------
    ⚠️ **代价不小**：低分位数让深度**系统性偏近**（窗口跨越倾斜面时取到近端边缘），
    三维距离被整体压缩 —— 上表里肩宽中位 0.24 比真值 0.40 小了 40%。

    > 📌 这个偏置与「飞点污染」（见 ``MIN_VALID_DEPTH_M``）**是两回事**，
    > 别混：偏置是**斜面上的真实几何**造成的，必然存在、只能调分位数权衡；
    > 飞点污染是**无效读数**混进来了，是缺陷，该被滤掉。
    > 2026-09-18 实测：滤掉飞点之后，这个偏置依然在（躺姿仍偏高约 0.4 m）。

    所以它必须和 ``anomaly.check_reprojection`` 一起用（那道门用的是**区间**），
    而且**门限要和这个偏置一起调**。本次没能找到两头都好的参数组合：
    小窗口无偏置但噪声大（IQR 0.86），大窗口噪声小但偏置大（误杀真样本）。
    详见 ``docs/倒地判据实测-方法B.md`` §4。

    ⚠️ 窗口取大是以**空间分辨率**换鲁棒性。两个靠得很近的人（互相遮挡）会互相污染。
    真机上如果目标密集，应按人的 bbox 限制窗口范围。

    Parameters
    ----------
    depth_m : np.ndarray
        米制深度图，无效值为 NaN（见 ``depth_to_meters``）。
    u, v : float
        像素坐标。
    radius : int
        方形窗口半径（像素）。17 -> 35x35。
    ...
    min_valid : int
        窗口内有效像素少于此数就返回 ``nan``。
        **在 ``min_depth_m`` 过滤之后统计** —— 窗口里全是飞点时算「没有有效像素」。
    min_depth_m : float
        有效量程下限（米）。低于它的像素按无效处理，见 ``MIN_VALID_DEPTH_M``。
    ...
    patch = depth_m[max(0, vi - radius):vi + radius + 1,
                    max(0, ui - radius):ui + radius + 1]
    # ⚠️ 「有效」不只是「有限」。低于量程下限的读数是**飞点**，不是测量值 ——
    # 它们全是极小值，而本函数取的是**近端**分位数，所以一旦占比超过 percentile
    # 就必定被选中。详见 MIN_VALID_DEPTH_M。
...
def project_depth_bbox(
    ...
    max_dispersion_m : float
        ...
        ⚠️⚠️ **这个门限的设计前提有问题，2026-09-17 在真实数据上实测后放宽到 3.0。**

        实测（低机位 RealSense、71 帧、目标距离 1.4~3.3 m）：

        ==================  =======  ====================  ======
        配置                中位数   四分位                最大
        ==================  =======  ====================  ======
        默认（原 0.5）       1.58     1.43 ~ 1.95           3.25
        换成 ``-seg`` + 掩膜  1.34     0.96 ~ 1.85           3.30
        ==================  =======  ====================  ======

        通过率：0.5 -> **0%**（全部拦掉）｜ 1.5 -> 32% ｜ 2.0 -> 74% ｜ 3.0 -> 89%

        也就是说 **0.5 的默认值让端到端跑出来是「零发布」**，
        而且不报任何错 —— 这正是对抗性审查预测到的失效模式（F12），
        已在真实链路上复现。

        **为什么掩膜也救不了**：这些目标距离只有 1.4~3.3 m，
        一个人身体自身的深度跨度就有 1.5 m 量级（头到脚、加躺姿）。
        固定绝对门槛区分不了「正常的人」和「框里混了两个差很远的面」——
        两者的离散度落在同一个量级。

        **所以这个门限现在只用来拦极端情况**（例如框里混进 10 m 外的墙）。
        它不是可靠的质量指标，真要判「这一团像素是不是同一个物体」，
        应当用掩膜 + 别的机制（例如深度直方图的多峰检测），
        而不是这一个标量门槛。**按实际场景标定它**，别照抄默认值。
    anchor : str
        ``"center"`` —— 3D 包围盒的中心。
        ``"bottom"`` —— 3D 包围盒**底面**中心。
