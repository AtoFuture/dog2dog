...
def bbox_aspect_is_fallen(
    ...
    ⚠️ **这只是粗筛，不能单独作为判据。** 受视角影响严重 ——
    俯视时站立的人也接近方形，换个机位就失效。

    这里做了一个粗略的俯仰角归一化：相机下俯 θ 时，远处站立的人的
    投影高度被压缩约 cos(θ)，所以先把观测到的比例除以 cos(θ) 再判。

    该近似忽略了目标自身的仰角，**只适用于目标接近地平线距离较远的情况**。
    近处目标会明显偏差 —— 这也是它只能当粗筛的原因。
