...
class FloorFit:
    ...
    # 变异测试实测：把 ``min_camera_height_m`` 的字段默认从 1.0 改成 0.0，
    # 全部测试照过，因为函数签名上还有另一份一模一样的默认值。
...
def fit_floor_plane(
    ...
    * **相机高度合理**：``normal · p + offset`` 定号后，相机到平面的**有符号**
    ...
    默认下限 **1.0 m** 是针对手持/固定安装的 Kinect 素材定的。**装在 Go2 上的
    相机会低得多**，那种场景必须把下限调小，否则每一帧都会被判不合格。
    """
