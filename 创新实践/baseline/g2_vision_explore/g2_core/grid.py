...
class GridMap:
    ...
    def clearance_m(self, precise: bool = True) -> np.ndarray:
        ...
        1. 传给 ``cv2.distanceTransform`` 的必须是 **traversable 掩膜**。
           ...
           ⚠️ **这一条与 ``docs/框架规划.md`` §4.3② 写的不一样，是有意的**
           （2026-09-18 审核发现两处说法冲突）：
           那份文档写「传参必须是 ``1 - occupied``」，而这里用 **traversable**
           （= 仅自由格），于是**未知格在这一道里等价于障碍**。

           选 traversable 的理由：未知区是「没看过的地方」，机器人不该贴着它走 ——
           与 frontier 检测里「图外 = 未知」的语义统一（见 ``frontier.frontier_mask``）。

           代价必须知道：**任何与未知区相邻的自由格，clearance 恒等于 1 格**。
           而 frontier 簇正好就是「贴着未知的那一圈自由格」，所以
           **不能**拿 frontier 格直接做安全半径校验 —— 那会一个候选都选不出来。
           ``explorer`` 用 ``goal_pullback_m`` 把目标点从边界退回自由空间来绕开它，
           见 ``explorer`` 模块 docstring 第 2 条。
